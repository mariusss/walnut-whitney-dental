...
export async function getAllCarouselImages(): Promise<CarouselImage[]> {
  ...
  const result = await db.select().from(carouselImages).orderBy(carouselImages.displayOrder);
  return result;
}

export async function insertCarouselImage(image: InsertCarouselImage): Promise<CarouselImage> {
  const db = await getDb();
  if (!db) {
  ...
  const inserted = await db.select().from(carouselImages).where(eq(carouselImages.id, insertedId)).limit(1);
  if (inserted.length === 0) {
    throw new Error("Failed to retrieve inserted carousel image");
  }
...
export async function updateCarouselImageOrder(id: number, displayOrder: number): Promise<void> {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
